...
// 🕒 ฟังก์ชันคืนค่าวันที่ปัจจุบันรูปแบบ YYYY-MM-DD
const getThaiToday = () => {
...
export default function Withdraw() {
    ...
    try {
      ...
      const formatted = data.map((item) => {
        return {
          name: item.prod_name,
          qty: item.quantity,
          fullDate: item.date ? item.date.split(' ')[0] : "",
          date: item.date, // เก็บวันที่เดิมจาก DB ไว้โชว์
          user_name: item.user_name || "ไม่ระบุชื่อ", // ดึงชื่อผู้ใช้จาก API
          note: item.note || "",
        };
      });
  ...
  const handlePreSubmit = () => {
    if (selectedItems.length === 0) return alert("กรุณาเลือกสินค้าที่ต้องการเบิก");
    for (let item of selectedItems) {
      const p = products.find((prod) => prod.id === item.id);
      const numQty = Number(item.qty);
      if (!item.qty || numQty <= 0) return alert(`กรุณาใส่จำนวนของ ${item.name}`);
      if (numQty > p.stock) return alert(`สินค้า ${item.name} ในคลังมีไม่พอ (คงเหลือ ${p.stock})`);
    }
    setIsModalOpen(true);
  };
  ...
  const confirmSubmit = async () => {
    setLoading(true);
    const nowLocal = new Date().toLocaleString('sv-SE', { timeZone: 'Asia/Bangkok' }).replace(' ', 'T');
            ...
            <div className="bg-white p-6 rounded-3xl shadow-sm border border-gray-200 flex flex-col h-[650px]">
              ...
              <div className="flex-1 overflow-y-auto pr-2 custom-scrollbar space-y-3 font-kanit">
                {filteredHistory.length === 0 ? (
                  <div className="text-center py-24 text-gray-300 font-kanit">
                  ...
                  filteredHistory.map((item, i) => (
                    <div key={i} className="bg-gray-50 p-4 rounded-2xl flex justify-between items-center border border-gray-50 hover:border-red-200 transition-all shadow-sm">
                      <div>
                        ...
                        {/* ✅ แก้ไขให้แสดงผลตามที่คุณส่งตัวอย่างมา */}
                        <p className="text-[10px] text-gray-400 uppercase tracking-widest mt-0.5 font-kanit">
                          👤 {item.user_name} · 📅 {item.date}
                        </p>
                        {item.note && <p className="text-xs text-red-400 mt-2 bg-red-50 px-2 py-1 rounded-lg w-fit font-kanit">📝 {item.note}</p>}
                      </div>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-md">
          <div className="bg-white w-full max-w-md rounded-[2.5rem] shadow-2xl p-8 text-center animate-pop-in font-kanit">
            <h3 className="text-2xl font-black mb-2 text-gray-800 font-kanit">ยืนยันการเบิกออก?</h3>
            <p className="text-gray-400 text-sm mb-6 font-kanit">กรุณาตรวจสอบรายการก่อนตัดยอดสต็อก</p>
            <div className="bg-gray-50 rounded-3xl p-5 mb-8 max-h-52 overflow-y-auto border text-left text-sm">
              {selectedItems.map((item) => (
                <div key={item.id} className="flex justify-between py-2 border-b border-dashed last:border-0 font-kanit">
            ...
            <div className="flex gap-4">
              ...
              <button onClick={confirmSubmit} disabled={loading} className="flex-1 py-4 rounded-2xl bg-black text-white font-bold transition-all shadow-lg font-kanit">
